...
class Wordler:
    def __init__(self, exit_to_menu_callback):
        pygame.init()
    ...
    # def create_two_player_tiles(self):
    #     for row in range(6):
    #         self.tiles.append([])
    #         for col1 in range(5):
    #             self.tiles[row].append(Tile((col1 * (TILESIZE + GAPSIZE)) + MARGIN_X1, (row * (TILESIZE + GAPSIZE)) + MARGIN_Y1))
    #         for col2 in range(5):
    #             self.tiles[row].append(Tile((col2 * (TILESIZE + GAPSIZE)) + MARGIN_X2, (row * (TILESIZE + GAPSIZE)) + MARGIN_Y2))
                    ...
                    if len(self.text) == 5:
                        ...
                        if self.text == self.word or self.current_row + 1 == 6:
                            ...
                            if self.text != self.word:
                                self.end_screen_text = UIElement(width * 0.33, height * 0.9, f"THE WORD WAS: {self.word}", white)

                            # player win, send win message
                            else:
                                self.end_screen_text = UIElement(width * 0.36, height * 0.9, "YOU GUESSED RIGHT", white)
