...
class Tile:
    ...
    def draw(self, screen, offset_x=0, offset_y=0):
        # Update the rect position based on current position
        self.rect.topleft = (self.x + offset_x, self.y + offset_y)
        ...
        if self.letter != "":
            self.font_x = self.x + (self.width / 2) - (self.font_width / 2) + offset_x
            self.font_y = self.y + (self.height / 2) - (self.font_height / 2) + offset_y
