...
def rps(server):
    ...
    text2 = font2.render("Not connected to server...", 1,  GREEN)
    ...
    while run:
        ...
        if game.bothWent():
            draw_rps_win(win, game, player, health_a, health_b)
...
def wordler(server):
    game = Wordler(exit_to_menu_callback=lambda: main_menu(server), victory=lambda: update_user_wordler_stats(stats))
    save_user_stats(username, stats)
...
def main_menu(server):
    ...
    pygame.display.set_caption("Multi-Game App")
    run = True
    clock = pygame.time.Clock()
    while run:
        clock.tick(60)
        ...
        RPS_BUTTON = mainButton(image=None, pos=(640, 210), 
                            text_input="Rock Paper Scissors", font=get_font(50), base_color= THE_OTHER_BLUE, hovering_color= white)
        SHOOTER_BUTTON = mainButton(image=None, pos=(640, 320), 
                            text_input="SHOOTER", font=get_font(50), base_color= THE_OTHER_BLUE, hovering_color= white)
        WORDLER_BUTTON = mainButton(image=None, pos=(640, 430), 
                            text_input="WORDLER", font=get_font(50), base_color= THE_OTHER_BLUE, hovering_color= white)
        STATS_BUTTON = mainButton(image=None, pos=(640, 540), 
                            text_input="STATS", font=get_font(50), base_color= THE_OTHER_BLUE, hovering_color= white)
        QUIT_BUTTON = mainButton(image=None, pos=(640, 650), 
                            text_input="QUIT", font=get_font(70), base_color= Dark_Red, hovering_color= white)
        

        win.blit(MENU_TEXT, MENU_RECT)

        for button in [RPS_BUTTON, SHOOTER_BUTTON, WORDLER_BUTTON, STATS_BUTTON, QUIT_BUTTON]:
