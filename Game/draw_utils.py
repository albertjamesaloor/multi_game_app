...
def draw_rps_win(win, game, p, health_a, health_b):
    ...
    if not(game.connected()):
        ...
        Player0_Health = font.render(f"Health: {health_a}", 1,  white)
        Player1_Health = font.render(f"Health: {health_b}", 1,  white)
        win.blit(Player0_Health, (10, 10))
        win.blit(Player1_Health, (width - Player0_Health.get_width() - 10, 10))
...
def draw_stats_win(win, stats):
    win.fill(BGCOLOUR)
    header_font = pygame.font.SysFont("Consolas", 60)
    stats_font = pygame.font.SysFont("Consolas", 40)
    ...
    win.blit(rps_total_games, (width/4 - rps_total_games.get_width()/2, 200))
    rps_wins = stats_font.render(f"Wins: {stats['rps_wins']}", 1,  white)
    win.blit(rps_wins, (width/4 - rps_wins.get_width()/2, 300))
    rps_losses = stats_font.render(f"Losses: {stats['rps_losses']}", 1,  white)
    win.blit(rps_losses, (width/4 - rps_losses.get_width()/2, 400))
    rps_win_rate = stats_font.render(f"Win Rate: {stats['rps_win_rate']}", 1,  white)
    win.blit(rps_win_rate, (width/4 - rps_win_rate.get_width()/2, 500))
    shooter_total_games = stats_font.render(f"Total Games: {stats['shooter_total_games']}", 1,  white)
    win.blit(shooter_total_games, (width*0.75 - shooter_total_games.get_width()/2, 300))
    shooter_red_wins = stats_font.render(f"Red Wins: {stats['shooter_red_wins']}", 1,  white)
    win.blit(shooter_red_wins, (width*0.75 - shooter_red_wins.get_width()/2, 400))
    shooter_yellow_wins = stats_font.render(f"Yellow Wins: {stats['shooter_yellow_wins']}", 1,  white)
    win.blit(shooter_yellow_wins, (width*0.75 - shooter_yellow_wins.get_width()/2, 500))
    wordler_guessed_words = stats_font.render(f"Total Guessed Words: {stats['wordler_guessed_words']}", 1,  white)
    win.blit(wordler_guessed_words, (width / 2 - wordler_guessed_words.get_width() / 2, 600))
    pygame.display.update()
